...
const goldMedalNumber = country => {
    return `SELECT COUNT(1) AS count FROM GoldMedal WHERE country = '${country}';`;
};
...
const mostSummerWins = country => {
  return `SELECT year, COUNT(id) AS count 
    FROM GoldMedal 
...
const bestDiscipline = country => {
  return `SELECT discipline, COUNT(id) AS count 
    FROM GoldMedal 
    WHERE country = '${country}' 
    GROUP BY 1 ORDER BY 2 DESC LIMIT 1;`;
};
...
const bestSport = country => {
  return `SELECT sport, COUNT(id) AS count 
    FROM GoldMedal 
    WHERE country = '${country}'
    GROUP BY 1 ORDER BY 2 DESC LIMIT 1;`;
};
...
const bestEvent = country => {
  return `SELECT event, COUNT(id) AS count 
    FROM GoldMedal 
    WHERE country = '${country}' 
    GROUP BY 1 ORDER BY 2 DESC LIMIT 1;`;
};
...
const mostMedaledAthlete = country => {
  return `SELECT name, COUNT(id) AS count 
    FROM GoldMedal 
    WHERE country = '${country}'
    GROUP BY 1 ORDER BY 2 DESC LIMIT 1;`;
};
...
const orderedMedals = (country, field, sortAscending) => {
  ...
  if(sortAscending) {
    query += `ASC`;
  }else if(sortAscending !== undefined && !sortAscending){
    query += `DESC`;
  }
...
const orderedSports = (country, field, sortAscending) => {
  let query = `SELECT sport, COUNT(sport) AS count, 
    (COUNT(1) * 100 / (SELECT COUNT(1) FROM GoldMedal WHERE country = '${country}')) as percent 
    FROM GoldMedal
    WHERE country = '${country}'
    GROUP BY 1 `;
